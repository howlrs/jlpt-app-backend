//! 重複検出用の共通ヘルパー。
//!
//! scripts リポジトリの `bin/dedup_common.rs` と同ロジック。
//! 管理者向けエンドポイント (`GET /api/admin/duplicates`) で利用する。
...
pub fn dedup_key(level_id: u32, sub: &SubLike) -> Result<String, KeySkipReason> {
    // すべての選択肢値を正規化
    let normalized: Vec<(String, String)> = sub
        .options
        .iter()
